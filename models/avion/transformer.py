...
class VisionTransformer(nn.Module):
    def __init__(
        ...
        if use_fast_conv1:
            self.conv1 = nn.Linear(
                in_features=3 * patch_size**2, out_features=width, bias=not ln_pre
            )
        else:
    ...
    def forward(self, x: torch.Tensor, return_feat=False, return_low_feat=False):
        if self.use_fast_conv1:
            if self.num_frames == 1:
                ...
                tile_temporal_embed = self.temporal_embedding.repeat_interleave(
        ...
        if self.image_projection is not None and not return_feat:
            x_cls = x_cls @ self.image_projection

        if return_feat:
            if return_low_feat:
                return x_cls, self.ln_post(x_feat), x_feat_low
            else:
                return x_cls, self.ln_post(x_feat)
        else:
            return x_cls
