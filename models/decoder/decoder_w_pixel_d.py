...
class MaskedTransformerDecoder(nn.Module):
    def __init__(
        ...
        self.num_depth_queries = num_queries # object query for depth estimation
        self.num_video_queries = cfg.MODEL.MASK2FORMER.VIDEO_QUERIES # global queries for video-text matching
        # self.use_map = cfg.MODEL.MASK2FORMER.USE_MAP
        self.use_map = False
        # learnable query features
        self.query_feat = nn.Embedding(self.num_video_queries + self.num_depth_queries, hidden_dim)
        # learnable query p.e.
        self.query_embed = nn.Embedding(self.num_video_queries + self.num_depth_queries, hidden_dim)
        # cross attention pooler for contrastive learning
        # if self.num_video_queries >= 1 and self.use_map:
        #     # self.pooled_token = nn.Parameter(torch.randn(1, hidden_dim))
        #     self.map_pooler = CrossAttention(dim=256, heads=8, dim_head=32)

        # level embedding (we always use 3 scales, use 1 for trial)
        self.num_feature_levels = cfg.MODEL.MASK2FORMER.NUM_FEATURE_LEVELS
    ...
    def forward(self, x, mask_features, mask=None):
        ...
        if self.num_video_queries >= 1 and self.use_map:
            glob_embedding = self.map_pooler(output[0], output[self.num_video_queries:]).squeeze(1)
        else:
            glob_embedding = output[0].unsqueeze(0)

        out = {
            "global_embed": glob_embedding,  # (2, 768)
    ...
    def forward_prediction_heads(self, outputs, mask_features, attn_mask_target_size):
        ...
        if self.num_video_queries > 0:
            outputs = outputs[self.num_video_queries:, :, :]  # depth queries
